...
public interface LicenseKeyRepository extends JpaRepository<LicenseKey, Long> {
  ...
  long countByProductIdAndVariantIdAndStatus(
    Long productId,
    Long variantId,
    KeyStatus status
  );
  ...
  Optional<LicenseKey> findFirstByProductIdAndStatus(
    Long productId,
    KeyStatus status
  );
  ...
  @Query(
    "SELECT k FROM LicenseKey k WHERE " +
      ...
      "AND (:productSearch IS NULL OR LOWER(k.product.title) LIKE LOWER(CONCAT('%', :productSearch, '%'))) " +
      "ORDER BY k.id DESC"
  )
