...
public class AdminCategoryServiceImpl implements AdminCategoryService {
  ...
  public CategoryResponse createCategory(CategoryRequest request) {
    ...
    if (categoryRepository.existsBySlug(slug)) {
      throw new RuntimeException(
        "Category with this slug already exists: " + slug
      );
    }
  ...
  public CategoryResponse updateCategory(Long id, CategoryRequest request) {
    ...
    if (request.getSlug() != null && !request.getSlug().isBlank()) {
      ...
      if (
        !newSlug.equals(c.getSlug()) && categoryRepository.existsBySlug(newSlug)
      ) {
    ...
    if (request.getDisplayOrder() != null) c.setDisplayOrder(
    ...
    if (productCount > 0) {
      throw new RuntimeException(
        "Cannot delete category with " +
          productCount +
          " products. Deactivate it instead."
      );
    }
